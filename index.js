  ...
  for (const file of eventFiles) {
    ...
    if (event.name && typeof event.execute === "function") {
      client.on(event.name, (...args) =>
        event.execute(...args, client, reactionRoleCache)
      );
...
require("./events/threadCreate.js")(client);

// Example debug event
client.on("messageReactionAdd", (reaction, user) => {
  console.log(
    "RAW messageReactionAdd event fired!",
    reaction.emoji.name,
    user.tag
  );
});
...
client.once("ready", async () => {
  ...
  console.log(
    `Loaded ${
      Object.keys(reactionRoleCache).length
    } reaction role panels into cache.`
  );

  // Set presence after the bot is ready
  client.user.setPresence({
    activities: [{ name: "With more bots 💛", type: ActivityType.Streaming }],
  });

  console.log(`Bot ${client.user.tag} is now ready!`);

  // Poll GitHub every 30 seconds
  setInterval(() => checkGitHubFeeds(client), 1000 * 5);
