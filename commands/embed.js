...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'create') {
      ...
      if (exists) {
        return interaction.reply({ content: `<a:zpyesno1:1368590377887469598> An 
embed named \`${name}\` already exists.`, ephemeral: false });
      }
    ...
    if (sub === 'edit') {
      ...
      if (!embedDoc) {
        return interaction.reply({ content: `<a:zpyesno1:1368590377887469598> No 
embed named \`${name}\` found.`, ephemeral: false });
      }
    ...
    if (sub === 'delete') {
      ...
      if (!deletedEmbed) {
        return interaction.reply({ content: `<a:zpyesno1:1368590377887469598> No 
embed found named \`${name}\`.`, ephemeral: false });
      }

      return interaction.reply({ content: `<a:zpyesno2:1368590432488915075> Embed \`$
...
embed named \`${name}\` found.`, ephemeral: false });
      ...
      return interaction.channel.send({ embeds: [embed] })
        .then(() => interaction.reply({ content: `<a:zpyesno2:1368590432488915075> 
Embed \`${name}\` sent successfully.`, ephemeral: false }))
        .catch(err => {
          console.error(err);
          interaction.reply({ content: `<a:zpyesno1:1368590377887469598> Failed to 
send embed \`${name}\`.`, ephemeral: false });
        });
    }
