...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('autothread')
    .setDescription('Configure and manage automatic thread creation in channels.')

    // Subcommand: /autothread channel config (launches channel picker menu)
    .addSubcommand(sub =>
      sub
        .setName('channel')
        .setDescription('Set which channels will auto-create threads for every message.')
        .addStringOption(opt =>
          opt
            .setName('config')
            .setDescription('Open the interactive channel select menu (click to continue)')
            .setRequired(true)
            .setChoices(
              { name: 'Open Channel Config Menu', value: 'open' }
            )
        )
    )
    ...
    .addSubcommand(sub =>
      sub
        .setName('edit')
        .setDescription('Edit the embed sent when a thread is auto-created in a specific channel.')
        .addStringOption(opt =>
          opt
            .setName('embed')
    ...
    .addSubcommand(sub =>
      sub
        .setName('threadname')
        .setDescription('Set the naming template for auto-created threads in a channel.')
        .addChannelOption(opt =>
          opt
            .setName('channel')
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'edit') {
      ...
      if (!config) {
        ...
        return interaction.reply({
          content: 'You must first configure channels for auto-threading with `/autothread channel config`.',
          ephemeral: true,
        });
      }

      // Check if channel is actually configured for auto-threading
      const chanConfig = config.channels.find(c => c.channelId === channel.id);
      if (!chanConfig) {
        return interaction.reply({
    ...
    if (subcommand === 'threadname') {
      ...
      return interaction.reply({
        content: `Thread naming template updated for <#${channel.id}>! Example: \`${template.replace('{user}', interaction.user.username).replace('{message}', '[message]')}\``,
        ephemeral: true,
      });
    }
  },
