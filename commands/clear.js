...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const fetched = await interaction.channel.messages.fetch({ limit: amount + 10 });
    ...
    if (!messagesToDelete || messagesToDelete.length === 0) {
      return interaction.reply({
        content: 'No messages found to delete (maybe not enough recent messages, or wrong filter).',
        ephemeral: true
      });
    }
    ...
    await interaction.channel.bulkDelete(messagesToDelete, true)
      ...
      .catch(err =>
        interaction.reply({
          content: 'Failed to delete messages. Make sure I have the right permissions and that messages are not too old.',
          ephemeral: true
        })
      );
  }
