...
module.exports = {
      ...
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("description")
            .setDescription(
              "Message (use {userId}, {level}, {username}, {mention})"
            )
            .setRequired(true)
        )
    )
  ...
  async execute(interaction) {
    ...
    if (sub === "profile") {
      ...
      const rewardRole = config?.levelRoles?.find(
    ...
    if (sub === "role-add") {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `Role <@&${role.id}> will now be awarded at level ${level}.`
        );
    ...
    if (sub === "reset") {
        ...
        return interaction.reply({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `${targetUser.username} has no leveling data to reset.`
              ),
          ],
          ephemeral: true,
        });
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle("Level Reset")
        .setDescription(
          `${targetUser.username}'s level and message count have been reset.`
        );
