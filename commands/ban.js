...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName("list").setDescription("List all ban cases in the server")
    )
    .addSubcommand((sub) =>
      sub
        .setName("view")
  ...
  async execute(interaction) {
    ...
    if (sub === "add") {
      ...
      try {
        ...
        return interaction.reply({
          content:
            "Failed to ban user. They may not exist, are already banned, or I lack permissions.",
          ephemeral: false,
        });
      }
    ...
    if (sub === "remove") {
      ...
      try {
        ...
        return interaction.reply({
          content:
            "Failed to unban user. They may not be banned, or I lack permissions.",
          ephemeral: false,
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `User ID: \`${targetId}\` has been unbanned and their ban cases removed.`
        )
        .setColor(0x57f287)
        .setTimestamp();
          ...
          userBan.bans.forEach((ban) => {
            lines.push(
              `**Case #${ban.case}** | User ID: ${userBan.userId}\n` +
                `> **Reason:** ${ban.reason}\n` +
                `> *by <@${ban.moderatorId}> on <t:${Math.floor(
                  new Date(ban.timestamp || Date.now()).getTime() / 1000
                )}:d>*\n`
            );
          });
        });
