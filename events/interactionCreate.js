...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isButton()) {
      ...
      const [prefix, action, section, embedId] =
  ...
  if (interaction.channel && interaction.channel.isThread()) {
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (prefix === "embed" && type === "modal") {
        ...
        if (section === "basic") {
          ...
          doc.color =
            emptyToNull(interaction.fields.getTextInputValue("color")) ||
            "#993377";
        }
      ...
      if (interaction.customId.startsWith("greeting_modal_embed_basic:")) {
        ...
        panel.greetingEmbed.title = emptyToNull(
          interaction.fields.getTextInputValue("title")
        );
        panel.greetingEmbed.description = emptyToNull(
          interaction.fields.getTextInputValue("description")
        );
        panel.greetingEmbed.color =
          emptyToNull(interaction.fields.getTextInputValue("color")) ||
          "#5103aa";
        await panel.save();
        return interaction.reply({
          content: "Greeting embed (basic info) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("greeting_modal_embed_author:")) {
        ...
        panel.greetingEmbed.author.name = emptyToNull(
          interaction.fields.getTextInputValue("author_name")
        );
        panel.greetingEmbed.author.icon_url = emptyToNull(
          interaction.fields.getTextInputValue("author_icon")
        );
        await panel.save();
        return interaction.reply({
          content: "Greeting embed (author) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("greeting_modal_embed_footer:")) {
        ...
        panel.greetingEmbed.footer.text = emptyToNull(
          interaction.fields.getTextInputValue("footer_text")
        );
        panel.greetingEmbed.footer.icon_url = emptyToNull(
          interaction.fields.getTextInputValue("footer_icon")
        );
        const timestampInput = interaction.fields
          .getTextInputValue("footer_timestamp")
          .toLowerCase();
        panel.greetingEmbed.footer.timestamp =
          timestampInput === "yes" || timestampInput === "true";
        await panel.save();
        return interaction.reply({
          content: "Greeting embed (footer) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("greeting_modal_embed_images:")) {
        ...
        if (!panel.greetingEmbed) panel.greetingEmbed = {};
        panel.greetingEmbed.thumbnail = emptyToNull(
          interaction.fields.getTextInputValue("thumbnail")
        );
        panel.greetingEmbed.image = emptyToNull(
          interaction.fields.getTextInputValue("image")
        );
        await panel.save();
        return interaction.reply({
          content: "Greeting embed (images) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_modal_embed_basic:")) {
        ...
        panel.embed.title = emptyToNull(
          interaction.fields.getTextInputValue("title")
        );
        panel.embed.description = emptyToNull(
          interaction.fields.getTextInputValue("description")
        );
        panel.embed.color =
          emptyToNull(interaction.fields.getTextInputValue("color")) ||
        ...
        await panel.save();
        return interaction.reply({
          content: "Ticket panel embed (basic info) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_modal_embed_author:")) {
        ...
        panel.embed.author.name = emptyToNull(
          interaction.fields.getTextInputValue("author_name")
        );
        panel.embed.author.icon_url = emptyToNull(
          interaction.fields.getTextInputValue("author_icon")
        );
        await panel.save();
        return interaction.reply({
          content: "Ticket panel embed (author) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_modal_embed_footer:")) {
        ...
        panel.embed.footer.text = emptyToNull(
          interaction.fields.getTextInputValue("footer_text")
        );
        panel.embed.footer.icon_url = emptyToNull(
          interaction.fields.getTextInputValue("footer_icon")
        );
        const timestampInput = interaction.fields
          .getTextInputValue("footer_timestamp")
        ...
        await panel.save();
        return interaction.reply({
          content: "Ticket panel embed (footer) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_modal_embed_images:")) {
        ...
        panel.embed.thumbnail = emptyToNull(
          interaction.fields.getTextInputValue("thumbnail")
        );
        panel.embed.image = emptyToNull(
          interaction.fields.getTextInputValue("image")
        );
        await panel.save();
        return interaction.reply({
          content: "Ticket panel embed (images) updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_modal_greeting:")) {
        ...
        panel.greetingEmbed =
          interaction.fields.getTextInputValue("greeting_text");
        await panel.save();
        return interaction.reply({
          content: "Greeting message updated.",
          ephemeral: true,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_modal_name:")) {
        ...
        const newName = interaction.fields
          .getTextInputValue("panel_name")
          .trim()
          .toLowerCase();
        const exists = await TicketPanel.findOne({
          guildId: interaction.guild.id,
          panelName: newName,
          _id: { $ne: panelId },
        });
        if (exists)
          return interaction.reply({
            content: `Panel name \`${newName}\` is already in use.`,
            ephemeral: false,
          });
        panel.panelName = newName;
        await panel.save();
        return interaction.reply({
          content: `Panel name updated to \`${newName}\`.`,
          ephemeral: true,
        });
      }
    }
    ...
    if (interaction.customId === "confession_submit") {
      ...
      const confessionText =
        interaction.fields.getTextInputValue("confessionText");
      ...
      const last = await Confession.findOne({ guildId }).sort({
        confessionId: -1,
      });
    ...
    if (interaction.isChannelSelectMenu()) {
      ...
      if (interaction.customId.startsWith("ticketpanel_select_post_channel:")) {
        ...
        await panel.save();
        return interaction.reply({
          content: `✅ Post channel set to <#${selectedChannelId}>.`,
          ephemeral: false,
        });
      }
      ...
      if (interaction.customId.startsWith("ticketpanel_select_category:")) {
        ...
        await panel.save();
        return interaction.reply({
          content: `✅ Ticket category set to <#${selectedCategoryId}>.`,
          ephemeral: false,
        });
      }
    }
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `Logging for **${eventType}** has been **disabled**.`
            ),
        ],
        components: [],
      });
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (customId === "perm_group_select") {
        return require("../events/permissionMenus").handleGroupSelect(
          interaction
        );
      }

      if (customId.startsWith("perm_command_select")) {
        return require("../events/permissionMenus").handleCommandSelect(
          interaction
        );
      }
    }
    ...
    if (interaction.isRoleSelectMenu()) {
      ...
      if (customId.startsWith("perm_role_select")) {
        return require("../events/permissionMenus").handleRoleSelect(
          interaction
        );
      }
    }
    ...
    if (interaction.customId === "perm_view_group_select") {
      const {
        commandGroups,
        groupLabels,
      } = require("./data/commandPermissions.js");
    ...
    if (
      ...
      interaction.customId.startsWith("perm_view_command_select")
    ) {
      ...
      if (!record || record.allowedRoles.length === 0) {
        embed.setDescription(
          "This command is **public** — no restrictions set."
        );
      } else {
        embed.setDescription("Restricted to the following roles:").addFields({
          name: "Allowed Roles",
    ...
    if (
      ...
      // 1. Archive configs for channels being removed this round:
      ...
      for (const removed of removedChannels) {
        if (
          !config.archivedChannels.some(
            (a) => a.channelId === removed.channelId
          )
        ) {
      ...
      // 2. Build new channels array, restoring settings if re-added
      const newChannels = [];
      for (const id of selectedChannelIds) {
        let found = config.channels.find((c) => c.channelId === id);
        if (!found && config.archivedChannels) {
          found = config.archivedChannels.find((c) => c.channelId === id);
        }
        if (found) {
          newChannels.push(found);
        } else {
          newChannels.push({
            channelId: id,
            embed: {},
      ...
      // 3. Assign new channels and clean up archive
      ...
      const confirmationEmbed = new EmbedBuilder()
        ...
        .setDescription(
          [
            `Auto-threading is now enabled for:\n${selectedChannelIds
              .map((id) => `<#${id}>`)
              .join(", ")}`,
            "",
            "**Tip:** You can safely re-add a channel at any time and your previous embed and thread settings will be restored.",
          ].join("\n")
        )
